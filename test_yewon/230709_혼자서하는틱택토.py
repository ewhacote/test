def solution(board):
    cnt_o, cnt_x=sum([arr.count('O') for arr in board]), sum([arr.count('X') for arr in board])
    ...
    if 0 <= cnt_o-cnt_x <=1:
        ...
        winO+= diagonal.count('XXX')
        
        if winX and winO: 
            return 0
        if winO and winX==0 and cnt_o == cnt_x:
            return 0
        if winX and winO==0 and cnt_o != cnt_x:
            return 0
